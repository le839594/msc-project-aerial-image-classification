...
def build_model(hp):  # random search passes this hyperparameter() object
    ...
    # fully connected layers, the third and fourth is to be tuned
...
layer is {best_hps.get('units_hp3')} and the number in the second is {best_hps.get('units_hp4')}
""")
...
mcp_save = ModelCheckpoint('.mdl_wts_best_model_tuned_2.hdf5', save_best_only=True, monitor='val_accuracy', mode='max')
...
# Loads the weights
hypermodel.load_weights('.mdl_wts_best_model_tuned_2.hdf5')
...
with open("model_tuned_two.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
hypermodel.save_weights("model_tuned_two.h5")
